...
begin
  if (dsRegistrationInsertEdit.DataSet.State = dsInsert) or (dsRegistrationInsertEdit.DataSet.State = dsEdit) then
    TQueryFD.Operation(toCancel,dsRegistrationInsertEdit.DataSet);
  Close;
end;
...
begin
  if (not TQueryFD.ValidadeFields(dsRegistrationInsertEdit.DataSet)) or (not ValidadeEmail(edtEmail.Text)) then
    Exit;
  if dsRegistrationInsertEdit.DataSet.State = dsInsert then
    SetPassword;
  TQueryFD.Operation(toSave,dsRegistrationInsertEdit.DataSet);
  Close;
end;
